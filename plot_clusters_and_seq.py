...
sequence_folder = "./data/sequences" # Directory containing cluster files 
...
seq_files = [file for file in os.listdir(sequence_folder) if file.startswith("seq")]

seq_data = {}

for file_name in seq_files:
    with open(os.path.join(sequence_folder, file_name), 'r') as f:
        seq = []
        for line in f:
            elements = line.strip().split(',')
            converted_elements = []
...
for file_name, cluster in cluster_data.items():
    ...
    print(f"column: {min_c}, {max_c}")
    print(f"row: {min_r}, {max_r}")
    print(CELLW, CELLH)
